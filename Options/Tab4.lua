local addonName,ns = ...
local L = ns.L
local DB = ns.PlateColorDB

ns.LazyBuild(ns.tabframe4, function()
--分页4滚动框架
local ConFramescrollFrame4 = CreateFrame("ScrollFrame", nil, ns.tabframe4, "ScrollFrameTemplate")
ConFramescrollFrame4:SetPoint("TOPLEFT", ns.tabframe4, "TOPLEFT", 4, -5)
ConFramescrollFrame4:SetPoint("BOTTOMRIGHT", ns.tabframe4, "BOTTOMRIGHT", -30, 5)
ConFramescrollFrame4:SetScript("OnMouseWheel", function(self, value)
	local step = 70
	local scroll = self:GetVerticalScroll()
	local range = self:GetVerticalScrollRange()
	if value > 0 then
		self:SetVerticalScroll(math.max(0, scroll - step))
	else
		self:SetVerticalScroll(math.min(range, scroll + step))
	end
end)
--分页4滚动内容
local ConFrame4 = CreateFrame("Frame", nil, ConFramescrollFrame4)
ConFrame4:SetSize(670,480)
ConFramescrollFrame4:SetScrollChild(ConFrame4)
ns.Y[4] = 0	--设置起始位置

ns.AddSetTiText(ConFrame4,4,L["NPC颜色作用于"])
local UseNpctable = {{L["无"],0},{L["名字"],1},{L["血条"],2},{L["名字+血条"],3}}
ns.AddSetDropdM(ConFrame4,4,L["NPC颜色作用于"],L["NPC颜色作用于"],UseNpctable,"UseNpc",ns.UpdateSetColor)

ns.AddSetTiText(ConFrame4,4,L["NPC分类"])
ns.AddClickColor(ConFrame4,4,L["等级>1颜色"],L["等级>1鼠标提示"],"NpcLv1","NpcLv1Color",ns.UpdateSetColor)
ns.AddClickColor(ConFrame4,4,L["BOSS颜色"],L["BOSS颜色"],"NpcLv2","NpcLv2Color",ns.UpdateSetColor)
local NpckickColor = ns.AddClickColor(ConFrame4,4,L["可打断NPC颜色"],L["可打断NPC颜色"],"Npckick","NpckickColor",ns.UpdateSetColor)
local NpcNokickColor = ns.AddClickColor(ConFrame4,4,L["不可打断NPC颜色"],L["不可打断NPC颜色"],"NpcNokick","NpcNokickColor",ns.UpdateSetColor)
NpcNokickColor.check:Disable()
NpcNokickColor.text:SetVertexColor(0.6,0.6,0.6)
NpckickColor.check:HookScript("OnClick",function(self)
	NpcNokickColor.check:SetChecked(self:GetChecked())
	PlateColorDB.NpcNokick = self:GetChecked()
end)
ns.AddClickColor(ConFrame4,4,L["可能是可打断NPC颜色"],L["可能是可打断NPC颜色"],"NpcSukick","NpcSukickColor",ns.UpdateSetColor)

end)